...
#functin to gather the training data through camera
...
#lets visualize the collected data
# Set the figure size
...
#now converting my data into a single list and then split it for training and testing 
...
#here we load the pretrained model NASNETmobile model
...
model.save("rps4.h5") #saving the model for the future use 

model = load_model("rps4.h5") #load the saved model for use
...
while True:
    ...
    cv2.rectangle(frame, (width - box_size, 0), (width, box_size), (0, 350, 250), 2)
...
#a simple function which tells the winner
...
#function to show the winner on the screeen
def show_winner(user_score, computer_score):    
    
    if user_score > computer_score:
        img = cv2.imread("youwin.jpg")
        
    elif user_score < computer_score:
        img = cv2.imread("comw.jpg")
...
#it will show the computer move
def display_computer_move(computer_move_name, frame):
    
    icon = cv2.imread( "{}.png".format(computer_move_name), 1)
...
box_size = 234
width = int(cap.get(3))
n=int(input("enter no. of rounds in the game : "))
...
while True:
    ...
    if prob >= confidence_threshold:
        ...
        final_user_move = counter.most_common(1)[0][0]
